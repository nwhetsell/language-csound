...
describe("language-csound", () => {
  beforeEach(() => {
    waitsForPromise(() => atom.packages.activatePackage("language-csound"));
  });
  ...
  describe("Csound grammar", () => {
    ...
    it("tokenizes quoted strings", () => {
      let {tokens} = grammar.tokenizeLine('"characters$MACRO."');
    ...
    it("tokenizes escape sequences", () => {
      ...
      let {tokens} = grammar.tokenizeLine(`"${escapeSequences.join("")}"`);
      ...
      tokens = grammar.tokenizeLine(`{{${escapeSequences.join("")}}}`).tokens;
      for (let i = 1; i < tokens.length - 1; i++) {
    ...
    it("tokenizes operators", () => {
      ...
      const lines = grammar.tokenizeLines(operators.join("\n"));
    ...
    it("tokenizes global value identifiers", () => {
      ...
      const lines = grammar.tokenizeLines(globalValueIdentifiers.join("\n"));
      for (let i = 0; i < lines.length; i++) {
        const tokens = lines[i];
        expect(tokens.length).toBe(1);
    ...
    it("tokenizes keywords", () => {
      ...
      const lines = grammar.tokenizeLines(keywords.join("\n"));
      for (let i = 0; i < lines.length; i++) {
        const tokens = lines[i];
        expect(tokens.length).toBe(1);
    ...
    it("tokenizes printks and prints escape sequences", () => {
      ...
      for (const opcode of opcodes) {
        const {tokens} = grammar.tokenizeLine(`${opcode} "${escapeSequences.join("")}"`);
    ...
    it("tokenizes goto statements", () => {
      ...
      // label.
      const lines = grammar.tokenizeLines(keywordsAndOpcodes.join(" aLabel //\n"));
    ...
    it("tokenizes include directives", () => {
      for (const character of ['"', "|"]) {
        const {tokens} = grammar.tokenizeLine(`#include/**/${character}file.udo${character}`);
        expect(tokens.length).toBe(6);
    ...
    it("tokenizes macro preprocessor directives", () => {
      ...
      const lines = grammar.tokenizeLines(preprocessorDirectives.join(" MACRO\n"));
      for (let i = 0; i < lines.length - 1; i++) {
        const tokens = lines[i];
        expect(tokens.length).toBe(3);
    ...
    it("tokenizes function-like macros", () => {
      const {tokens} = grammar.tokenizeLine("$MACRO.(((x#y\\)))' \"(#'x)\\)x\\))\"# {{x\\))x)\\)(#'}});");
  ...
  describe("Csound Score grammar", () => {
    ...
    it("tokenizes repeat section statements", () => {
      const {tokens} = grammar.tokenizeLine("r 1 MACRO\n");
      expect(tokens.length).toBe(6);
